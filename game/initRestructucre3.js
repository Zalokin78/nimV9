...
const localStorageManagerTest = function (genStats) {
  ...
  const currTokenArr = tokenObjectArrayGen();
  console.log(genStats.tokenList);
  ...
  const testArr1Str = JSON.stringify(testArr1);

  console.log(testArr1Str);

  const testArrUnStr = JSON.parse(testArr1Str);

  console.log(testArrUnStr);

  //array inside an object test

  //creation of an object, adding objects into an array DYNAMICALLY
  const testObj = { name: "Nicholas", surname: "Galliano", testArr: [] };

  const x = {};

  /* for (let i = 0; i < 5; i++) {
    console.log(i);
    let x = new Object(bob);
    x.age = 10 + i;
    x.weight = 60 + i;
    testObj.testArr.push(bob);
  } */
  ...
  const testObjUnStr = JSON.parse(testObjStr);

  console.log(testObjUnStr);
  ...
  /* const testObj1Str = JSON.stringify(testObj1); */

  const testObj1Str = JSON.stringify(testObj1);

  console.log(testObj1Str);
