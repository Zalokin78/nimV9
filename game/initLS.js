...
const localStorageManager = function (genStats, action) {
  ...
  if (action == "write") {
    const genStatsEntries = Object.entries(genStats);
    console.log(genStatsEntries);
    /* genStatsEntries.forEach((element) => {
      const [x, y] = element;
      LSMap.set(x, y);
    });
    console.log(LSMap); */
    let tokenArray = [];
    console.log(genStatsEntries[0][1]);
    genStatsEntries[0][1].forEach((element) => {
      /* const [x, y] = element;
      console.log(x, y); */
      tokenArray.push(element.id)[0];
      tokenArray.push(element.hidden)[1];
    });
    console.log(tokenArray);
    let tokenArray1 = [[], []];

    genStatsEntries.forEach((element) => {
      tokenArray1[0].push(element)[0];
      tokenArray1[1].push(element)[1];
    });
    console.log(tokenArray1);
    console.log(genStatsEntries.flat(1));
  }
...
const fakeLSMap = new Map();
fakeLSMap.set("0A", true);
fakeLSMap.set("1A", false);
fakeLSMap.set("1B", true);
fakeLSMap.set("1C", false);
fakeLSMap.set("2A", false);
fakeLSMap.set("2B", false);
fakeLSMap.set("2C", false);
fakeLSMap.set("2D", true);
fakeLSMap.set("2E", false);
fakeLSMap.set("3A", false);
fakeLSMap.set("3B", true);
fakeLSMap.set("3C", false);
fakeLSMap.set("3D", true);
fakeLSMap.set("3E", true);
fakeLSMap.set("3F", false);
fakeLSMap.set("3G", false);
