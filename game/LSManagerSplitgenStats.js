...
const localStorageRead = function () {
  /* let genStats = {}; */
  /* if (localStorage.getItem("genStatsStr") == !null) {
    console.log("Let's do this!");
    genStats = JSON.parse(localStorage.getItem("genStatsStr"));
  } else {
  ...
  /* const { tokenList, ...genStats } = genStatsAll; */

  //need to split - to place the tokenList items inside the token nodes (need to separate
  //tokenList from the genStat obj)
  //not sure if this is the way to go - i think that the split should happen in genToken()
  //function
  console.log(testGenStats);
  /* console.log(tokenList); */
  /* return [genStats, tokenList]; */
  return genStats;
};
...
const genTokenNodes = function (genStats) {
  ...
  for (let rowNum = 0; rowNum < rowLimit; rowNum++) {
    for (let x = 0; x < xMax; x++) {
      ...
      //careful here when entering from an exisiting game
      /* if (localStorage.getItem("refreshBoard") == "no") {
        token.locked = tokenList[tokenNum].locked;
        token.hidden = tokenList[tokenNum].hidden;
      } else {
        token.locked = false;
        token.removed = false;
      } */
      ...
      /* genStats.tokenList[tokenNum].push(token.removed) = false
      genStats.tokenList[tokenNum].push(token.locked) = false */
